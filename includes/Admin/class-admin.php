...
class TAKA_Platform_Admin {
	...
	private static function import_config( $mode, $dry_run, $delete_existing, $config = null ) {
		...
		if ( class_exists( 'TAKA_Ticketing_Module' ) && ! empty( $config['ticketing'] ) && is_array( $config['ticketing'] ) ) {
			TAKA_Ticketing_Module::import_ticketing_config( $config['ticketing'], $mode, $dry_run );
		}
	...
	private static function delete_plugin_posts() { foreach ( array_filter( array( TAKA_PLATFORM_CPT_EVENT, TAKA_PLATFORM_CPT_ORGANIZER, TAKA_PLATFORM_CPT_VENUE, defined( 'TAKA_PLATFORM_CPT_TOUR_PLANNING' ) ? TAKA_PLATFORM_CPT_TOUR_PLANNING : '', defined( 'TAKA_PLATFORM_CPT_TICKET_PROMOTION' ) ? TAKA_PLATFORM_CPT_TICKET_PROMOTION : '' ) ) as $type ) { $ids = get_posts( array( 'post_type' => $type, 'post_status' => 'any', 'posts_per_page' => -1, 'fields' => 'ids' ) ); foreach ( $ids as $id ) { wp_delete_post( $id, true ); } } }
