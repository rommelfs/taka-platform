...
class TAKA_Platform_Data {
	...
	/** Full country choices for person-facing forms; configured country labels override the generic ISO labels. */
	public static function country_choices( $lang = null, $include_configured_overrides = true ) {
		$choices = self::default_country_choices();
		if ( $include_configured_overrides ) {
			$lang = $lang ?: self::platform_fallback_language();
			foreach ( self::get_option_lists( false )['country']['options'] ?? array() as $option ) {
				$code = strtoupper( trim( (string) ( $option['key'] ?? '' ) ) );
				$label = self::option_label_for_language( $option, $lang );
				if ( 2 === strlen( $code ) && '' !== trim( (string) $label ) ) {
					$choices[ $code ] = (string) $label;
				}
			}
		}
		natcasesort( $choices );
	...
	private static function default_country_choices() {
		return array(
			'AF' => 'Afghanistan',
			'AX' => 'Aland Islands',
			'AL' => 'Albania',
	...
	public static function country_code_for_value( $value ) {
		...
		if ( 2 === strlen( $code ) && isset( self::default_country_choices()[ $code ] ) ) {
			return $code;
		}
	...
	public static function country_label( $value, $lang = null ) {
		$value = trim( (string) $value );
		if ( '' === $value ) { return ''; }
		$configured = self::resolve_option_list_label( 'country', $value, $lang, '' );
		if ( '' !== trim( (string) $configured ) ) {
			return $configured;
		}
		$code = self::country_code_for_value( $value );
		$choices = self::country_choices( $lang, false );
		return isset( $choices[ $code ] ) ? $choices[ $code ] : $value;
	}
	...
	public static function default_hero_settings() {
		...
		return array(
			...
			'description'            => taka_tour_translate( 'hero.intro', 'Eine europäische Seminarreise mit Takafumi \'Taka\' Nakayama Sensei – von Helsinki über Berlin, die Niederlande, Belgien und Luxemburg bis in die Region Trier/Konz.' ),
	...
	public static function default_content_sections() {
		...
		return array(
			'sensei'   => self::normalize_content_section( array( 'key' => 'sensei', 'visible' => '1', 'sort_order' => 10, 'kicker' => taka_tour_translate( 'sections.sensei.kicker', 'Sensei' ), 'title' => taka_tour_translate( 'sections.sensei.headline', 'Takafumi \'Taka\' Nakayama' ), 'subtitle' => '', 'body' => taka_tour_translate( 'sections.sensei.text', 'Präzision, Ruhe und Bewegungsqualität aus der okinawanischen Tradition.' ), 'image_id' => 0, 'image_url' => $images['taka_portrait'] ?? '', 'secondary_image_id' => 0, 'secondary_image_url' => '', 'gallery_image_ids' => array(), 'gallery_image_urls' => array(), 'layout' => 'image_right', 'background_style' => 'paper', 'button_url' => '', 'button_label' => '', 'css_class' => '', 'image_fit' => 'contain', 'image_position' => 'center center' ) ),
				...
				array(
					...
					'translations' => array(
						'de' => array(
							...
							'subtitle' => '',
							'body' => 'Wenn du als Dojo-Inhaber oder Organisator ein zukünftiges Seminar mit Takafumi \'Taka\' Nakayama Sensei ausrichten möchtest, melde dich gerne. Die Route 2026 ist möglicherweise bereits geplant, aber zukünftige Touren ab 2027 können wir gemeinsam besprechen.',
						...
						'en' => array(
							...
							'subtitle' => '',
							'body' => 'If you are a Dojo owner or organizer and would like to host a future seminar with Takafumi \'Taka\' Nakayama Sensei, get in touch. The 2026 route may already be planned, but future tours from 2027 onward can be discussed.',
						...
						'fr' => array(
							...
							'subtitle' => '',
							'body' => 'Si tu es propriétaire de Dojo ou organisateur et souhaites accueillir un futur séminaire avec Takafumi \'Taka\' Nakayama Sensei, contacte-nous. Le parcours 2026 est peut-être déjà planifié, mais les futures tournées à partir de 2027 peuvent être discutées.',
						...
						'nl' => array(
							...
							'subtitle' => '',
							'body' => 'Ben je Dojo-eigenaar of organisator en wil je in de toekomst een seminar met Takafumi \'Taka\' Nakayama Sensei hosten? Neem contact op. De route voor 2026 is mogelijk al gepland, maar toekomstige tours vanaf 2027 kunnen we bespreken.',
						...
						'lb' => array(
							...
							'subtitle' => '',
							'body' => 'Bass du Dojo-Besëtzer oder Organisateur a wëlls an Zukunft e Seminar mam Takafumi \'Taka\' Nakayama Sensei organiséieren? Mell dech gär. D’Route 2026 ass eventuell schonn geplangt, mee zukünfteg Touren ab 2027 kënne mir zesumme beschwätzen.',
						...
						'fi' => array(
							...
							'subtitle' => '',
							'body' => 'Jos olet Dojon omistaja tai järjestäjä ja haluaisit isännöidä tulevan seminaarin Takafumi \'Taka\' Nakayama Sensein kanssa, ota yhteyttä. Vuoden 2026 reitti voi olla jo suunniteltu, mutta tulevista kiertueista vuodesta 2027 alkaen voidaan keskustella.',
						...
						'ja' => array(
							...
							'subtitle' => '',
							'body' => 'Dojoの責任者または主催者として、Takafumi \'Taka\' Nakayama Sensei の今後のセミナー開催に関心があればご連絡ください。2026年のルートはすでに計画済みの場合がありますが、2027年以降のツアーについて相談できます。',
