...
| Door price additional note | `_taka_ticket_door_note` / `ticket_door_note` | single-line | yes, per Event | yes, directly with that Event's pay-at-door price when non-empty | translation packages and Event exports | yes | keep next to the Door price; never use a global value |
